...
read -p "Do you want to convert these files to DOS format? (y/n) " confirm
if [ $confirm = "y" ]; then
    echo "Converting files to DOS format..."
    # Convert each file with the ".sh" extension to DOS format and list them
    find "$dir_path" -maxdepth 1 -type f -name '*.sh' -exec unix2dos {} \;
else
